...
# ============ KHUNG/MÀU THÔNG BÁO ============
# _email_* dựng 1 "thẻ" nền trắng cố định (không phụ thuộc theme sáng/tối của
# backend) chứa: lời chào, badge trạng thái theo màu, và nút CTA — đổi màu
# brand thì sửa duy nhất EMAIL_BRAND_COLOR (đồng bộ tay với backend.css, vì
# email thật không đọc được biến CSS). Dùng SONG SONG class (o_eaut_notif_*,
# định nghĩa ở static/src/css/backend.css) + style inline: message_post() sẽ
# strip style khi hiển thị trong chatter — lúc đó class từ backend.css gánh
# phần hiển thị; còn với email thật gửi ra ngoài (Gmail/Outlook...) thì class
# vô nghĩa (client không tải CSS ngoài) nên vẫn cần giữ style inline.
...
def _email_body(greeting_name, paragraphs, cta_url=None, cta_label=None):
    """Dựng nội dung thông báo bên trong 1 "thẻ" nền trắng cố định — lời chào +
    các đoạn nội dung (mỗi đoạn tự chèn badge nếu cần, xem các nơi gọi) + 1 nút
    CTA cuối (nếu có). paragraphs: list các đoạn Markup, đoạn nào rỗng/None bị
    bỏ qua — tiện cho trường hợp không có "reason" (VD: submit lần đầu)."""
    html = Markup(
    ...
    return Markup(
        '<div class="o_eaut_notif_card" style="background:#ffffff;'
        'border:1px solid #e0e0e0;border-radius:8px;padding:16px 20px;">%s</div>'
    ) % html
...
class ShowcaseAdvisorRegistration(models.Model):
    ...
    def _activate_next_line(self, reason=None):
        ...
        if not next_line:
            ...
            self.message_post(body=body, partner_ids=self.student_id.ids, sanitize=False)
            return
    ...
    def action_reset_for_withdrawal(self, creator=None):
        ...
        for reg in self:
            ...
            if creator:
                ...
                reg.message_post(body=body, partner_ids=reg.student_id.ids, sanitize=False)
    ...
    def _notify(self, partner, body):
        ...
        if partner:
            self.registration_id.message_post(
                body=body, partner_ids=partner.ids, sanitize=False)
